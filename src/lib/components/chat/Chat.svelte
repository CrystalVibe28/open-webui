		...
		if (chat) {
			...
			if (chatContent) {
				...
				if (pendingTaskIds.length > 0 && !responseComplete) {
					...
					if (currentMessage?.id && !hasPendingAskUser) {
						clearPendingModelContextUsage([currentMessage.id]);
					}
					// No active tasks and message incomplete → generation was interrupted
					if (currentMessage?.role === 'assistant' && !currentMessage.done && !hasPendingAskUser) {
